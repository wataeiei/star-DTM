...
## 2. Build the fixed 2,520-image split
...
- 14 images per class for validation: 420 total.
- 14 images per class for final in-domain testing: 420 total.
- No selected train/validation/test pair has identical RGB pixels.
...
```bash
find data/aid2520/train_hr -type f | wc -l
find data/aid2520/val_hr -type f | wc -l
find data/aid2520/test_hr -type f | wc -l

cat data/aid2520/split_manifest_summary.json
...
test = pd.read_csv(f"{root}/test_eval_manifest.csv")
...
print("test classes:", test["class_name"].nunique())
sets = [set(frame.pixel_sha256) for frame in (train, val, test)]
print("train/val overlap:", len(sets[0] & sets[1]))
print("train/test overlap:", len(sets[0] & sets[2]))
print("val/test overlap:", len(sets[1] & sets[2]))
...
print(test.groupby("class_name").size().to_string())
EOF
```
...
python3 ucmerced_semantic_classifier.py train \
  --train_manifest data/aid2520/train_manifest.csv \
  --output_dir outputs/aid2520_resnet18_classifier \
  --epochs 40 \
  --batch_size 32 \
  --image_size 224 \
...
python3 ucmerced_semantic_classifier.py eval \
  ...
  --eval_manifest data/aid2520/test_eval_manifest.csv \
  --source HR=data/aid2520/test_hr \
  --output_dir outputs/aid2520_resnet18_classifier_hr_test \
  --batch_size 64 \
  --dtype bf16 \
  --num_workers 2

cat outputs/aid2520_resnet18_classifier_hr_test/semantic_metrics_summary.csv
```

Use the same `data/aid2520` split for every SR method and every training seed.
